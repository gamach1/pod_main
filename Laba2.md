...
Используемый стек технологий Язык программирования: Python 3.8.10
...
Теоретические сведения MapReduce — это модель и фреймворк для параллельной обработки больших массивов данных в кластере. Вычисления делятся на два основных этапа:
Map (Отображение) — обработка входных данных и генерация промежуточных пар «ключ-значение»

Reduce (Свёртка) — агрегирование промежуточных значений по ключам
...
Файл input.txt:
hello world hello data goodbye data big data analytics hello hadoop mapreduce programming

Файл grades.txt:
101,math,92 101,physics,85 102,math,90 102,physics,95 103,math,80 103,physics,88 104,math,75 104,physics,82
...
cat grades.txt | python3 mapper_avg.py | sort | python3 reducer_avg.py Результат:
...
hadoop fs -put input.txt /user/student/input_data/ hadoop fs -put grades.txt /user/student/input_data/
...
Проверка результатов:
hadoop fs -cat /user/student/output_avg/part-00000 5. Результаты выполнения

5.1. Результаты WordCount Количество слов analytics 1 big 1 data 3 goodbye 1 hadoop 1 hello 3 mapreduce 1 programming 1 world 1
